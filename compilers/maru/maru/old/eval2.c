...
enum { Undefined, Long, String, Symbol, Pair, _Array, Array, Expr, Form, Subr };
...
static oop globals= nil, evaluators= nil, applicators= nil;
...
static oop eval(oop obj, oop env)
{
  ...
  switch (getType(obj)) {
    ...
    case Pair: {
      ...
      if (is(Symbol, head)) {
	if (s_if == head) {		// (if tst seq alt)
	  return eval(((nil == eval(cadr(obj), env)) ? cadddr(obj) : caddr(obj)), env);
	}
	if (s_and == head) {		// (and t1 t2 ...)
	  oop ans= s_t;
	  for (obj= getTail(obj);  is(Pair, obj);  obj= getTail(obj))
	    if (nil == (ans= eval(getHead(obj), env))) break;
	  return ans;
	}
	if (s_or == head) {		// (or t1 t2 ...)
	  oop ans= nil;
	  for (obj= getTail(obj);  is(Pair, obj);  obj= getTail(obj))
	    if (nil != (ans= eval(getHead(obj), env))) break;
	  return ans;
	}
	if (s_set == head) {		// (set var val)
	  oop var= assq(cadr(obj), env);
	  if (!is(Pair,var)) {
	    printf("cannot set undefined variable: ");
	    dumpln(cadr(obj));
	    exit(1);
	  }
	  return setTail(var, eval(caddr(obj), env));
	}
	if (s_let == head) {
	...
	if (s_while == head) {
	  oop tst= car(getTail(obj));
	  while (nil != eval(tst, env)) {
	    oop body= cdr(getTail(obj));
	    while (is(Pair, body)) {
	...
	if (s_quote == head) {
	  return car(getTail(obj));
	}
	if (s_lambda == head) {
	  return newExpr(getTail(obj), env);
	}
	if (s_define == head) {
	  oop symbol= cadr(obj);
	  if (!is(Symbol, symbol)) {
	  ...
	  oop value= eval(caddr(obj), env);		GC_PROTECT(value);
	...
	head= apply(head, getTail(obj), env);
	head= eval(head, env);				GC_UNPROTECT(head);
	return head;
      }
    ...
    default: {
      ...
      if (nil != ev) return apply(ev, obj, env);
      return obj;
    }
  }
  return nil;
...
#undef _do
  ...
  tmp= newSubr(subr_exit);		define(intern("exit"),		tmp, globals);
  tmp= newSubr(subr_apply);		define(intern("apply"), 	tmp, globals);
  tmp= newSubr(subr_type_of);		define(intern("type-of"), 	tmp, globals);
  ...
  tmp= newSubr(subr_array);		define(intern("array"),		tmp, globals);
  tmp= newSubr(subr_array_at);		define(intern("array-at"),	tmp, globals);
  tmp= newSubr(subr_set_array_at);	define(intern("set-array-at"),	tmp, globals);
